...
describe('computeStickAxis', () => {
  it('returns a centred neutral axis when the pointer is at the stick centre', () => {
    const r = rect();
    expect(computeStickAxis(r, 160, 160)).toEqual({ x: 0, z: 0 });
  });

  it('clamps to the unit circle at the edges and beyond', () => {
    const r = rect();
    expect(computeStickAxis(r, 220, 160)).toEqual({ x: 1, z: 0 });
    expect(computeStickAxis(r, 1000, 160)).toEqual({ x: 1, z: 0 });
    expect(computeStickAxis(r, 100, 40)).toEqual({ x: -1, z: -1 });
  });
  ...
  it('returns neutral instead of NaN/Infinity when the rect has zero width', () => {
    ...
    expect(computeStickAxis(rect({ width: 0 }), 160, 160)).toEqual({ x: 0, z: 0 });
  });

  it('returns neutral for a missing/undefined rect', () => {
    expect(computeStickAxis(undefined, 160, 160)).toEqual({ x: 0, z: 0 });
  });

  it('returns neutral for non-finite pointer coordinates', () => {
    const r = rect();
    expect(computeStickAxis(r, NaN, 160)).toEqual({ x: 0, z: 0 });
    expect(computeStickAxis(r, Infinity, 160)).toEqual({ x: 0, z: 0 });
    expect(computeStickAxis(r, 160, -Infinity)).toEqual({ x: 0, z: 0 });
  });
